...
import EditGallery from "../../components/offerts/EditGallery";
...
const Edit = () => {
  ...
  async function handleSend() {
    try{
      ...
      const result = await axiosPrivate(auth.accessToken).put(`offerts/update/${offerId}`,
        form,
        {headers: { 'Content-Type': 'multipart/form-data' }}
      )
      if ( result.status === 200 ) {
        navigate('/a/offers/details', {state: {id: offerId}, replace: true})
      }
    }catch (error){
      console.error(error)
    }
  ...
  useEffect(()=> {
    async function get(id) {
      try {
        const result = await axiosPrivate(auth.accessToken).get(`/offerts/details/${id}`);
        ...
        if ( result.status === 200 ){
          ...
          setFormData({
            ...
            category: data.car.category[0],
            carColor: data.car.color,
            carFuelType: data.car.engine.fuelType,
            carPower: data.car.engine.power,
            carEngCapacity: data.car.engine.capacity,
            carDrive: data.car.drive,
            carTrans: data.car.transmission,
            carGears: data.car.gears,
            carDoors: data.car.doorsNumber,
            carNumberPlate: data.car.numberPlate,
            functionalities: data.functionalities,
            vin: data.car.vin,
            odometer: data.car.odometer,
            salon: data.salons._id,
            oldPhotos: data.gallery,
    ...
    <div className="w-full flex justify-center">
      <div className='w-full max-w-4xl'>
        <div className='p-2 shadow-lg mb-3 rounded-md bg-white flex justify-between items-center'>
          <div>
            <BackButton path={"/a/offers/details"} prevState={{id: offerId}}></BackButton>
          </div>
          <div>
            <FormControl className='text-lg flex justify-center md:justify-end items-end'>
              <div className='flex gap-4 justify-center'>
                <Checkbox value={formData.isActive} checked={formData.isActive} onChange={handleChecked}>Aktywna</Checkbox>
        ...
        <div className='p-2 shadow-lg rounded-md bg-white flex flex-col gap-2'>
          <div>
            <h1 className='text-center text-2xl text-gray-600 font-semibold'>Edycja Oferty</h1>
          ...
          <div className="mt-4">
            ...
            <div className="mt-4">
              ...
              <div className='w-full'>
                {
                  formData?.oldPhotos.length !== 0 &&
                  <EditGallery  form={formData} setForm={setFormData}/>
                }
              </div>
            </div>
